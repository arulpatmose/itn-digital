                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <?php if ($b['status'] === 'pending'): ?>
                                                <div class="btn-group">
                                                    <?php if (auth()->user()->can('booking.approve')): ?>
                                                        <button type="button"
                                                            class="btn btn-sm btn-success btn-approve-booking"
                                                            data-id="<?= $b['id'] ?>"
                                                            data-bs-toggle="tooltip" title="Approve">
                                                            <i class="fa fa-check"></i>
                                                        </button>
                                                        <button type="button"
                                                            class="btn btn-sm btn-danger btn-reject-booking"
                                                            data-id="<?= $b['id'] ?>"
...
$(function () {
    ...
    if ($('#table-bookings').length) {
        $('#table-bookings').DataTable({
            ...
            columnDefs: [
                ...
                { targets: 'resourceName', width: '22%' },
                { targets: 'timeSlot', width: '16%' },
                { targets: 'purposeName', width: '14%' },
                { targets: 'requestedBy', width: '16%' },
                { targets: 'bookingStatus', width: '8%', className: 'text-center' },
    ...
    $('#btn-confirm-booking-action').on('click', function () {
        var url     = _AppUri + '/bookings/' + _pendingAction;
        var remarks = $('#approval-remarks').val();

        $.ajax({
            url: url,
            type: 'POST',
            data: { id: _pendingId, approval_remarks: remarks },
            dataType: 'json',
